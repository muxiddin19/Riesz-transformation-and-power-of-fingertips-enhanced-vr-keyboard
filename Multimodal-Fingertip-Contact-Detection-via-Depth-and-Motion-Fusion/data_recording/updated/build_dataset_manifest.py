...
def process_session(session_dir):
    ...
    for label_file in label_files:
        ...
        for label_hand, ann_hand in zip(label_hands, ann_hands):
            ...
            # PATCH: generate_contact_labels.py now labels fingertips
            # "tap"/"touch"/"hover" (not a single "contact" state), and
            # writes a "has_contact" boolean alongside for exactly this
            # kind of downstream consumer. Fall back to the old
            # state == "contact" check for labels/ written before that
            # change and never regenerated.
            def _is_contact(finfo):
                if "has_contact" in finfo:
                    return finfo["has_contact"]
                return finfo.get("state") == "contact"

            has_contact = any(_is_contact(f) for f in fingertips.values())

            clicked_key = None
            for finger, finfo in fingertips.items():
                if _is_contact(finfo):
