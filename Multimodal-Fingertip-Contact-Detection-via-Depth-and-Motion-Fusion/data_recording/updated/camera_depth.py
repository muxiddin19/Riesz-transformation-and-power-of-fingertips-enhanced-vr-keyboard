...
class DepthCamera:
    def __init__(self, name: str, width: int = 848, height: int = 480, fps: int = 30):
        ...
        self.depth_filter = DepthFilter(min_depth_m=min_depth_m, max_depth_m=max_depth_m)

        # PATCH: recorded once here so metadata.json can carry the exact
        # filter settings a session was captured with, instead of leaving
        # readers to assume they match whatever DepthFilter's current
        # tuning happens to be.
        self.filter_params = {
            "decimation_magnitude": 1,
            "spatial_magnitude": DepthFilter.SPATIAL_MAGNITUDE,
            "spatial_smooth_alpha": DepthFilter.SPATIAL_SMOOTH_ALPHA,
            "spatial_smooth_delta": DepthFilter.SPATIAL_SMOOTH_DELTA,
            "temporal_smooth_alpha": DepthFilter.TEMPORAL_SMOOTH_ALPHA,
            "temporal_smooth_delta": DepthFilter.TEMPORAL_SMOOTH_DELTA,
            "hole_fill_mode": DepthFilter.HOLE_FILL_MODE,
    ...
    def start(self):
        ...
        # PATCH: real intrinsics of the color stream -- this is the
        # profile that both the saved color frames AND the depth frames
        # (aligned to color, right below) actually share, so it's the
        # correct fx/fy/cx/cy for downstream 3D reconstruction from either.
