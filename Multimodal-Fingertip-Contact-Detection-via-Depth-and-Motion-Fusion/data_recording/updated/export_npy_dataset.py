...
# PATCH: target size every camera's frames get resized to before stacking.
# Cameras don't all capture at the same native resolution (e.g. cam2_30/
# cam3_60 were dropped to 320x240 to fix USB bandwidth stalls, while
# cam1_90/depth_cam stay at 640x480) -- and that resolution can even change
# for the SAME camera across sessions recorded before/after such a fix.
# np.stack() requires uniform shape within a camera's array, so every
# frame is resized to this common target regardless of what was natively
# captured.
EXPORT_WIDTH = config.RGB_WIDTH
EXPORT_HEIGHT = config.RGB_HEIGHT
    ...
    for r in rows:
        ...
        for cam in camera_names:
            ...
            if rgb is None or depth is None:
                ...
                ok = False
                break
            if rgb.shape[:2] != (EXPORT_HEIGHT, EXPORT_WIDTH):
                rgb = cv2.resize(rgb, (EXPORT_WIDTH, EXPORT_HEIGHT), interpolation=cv2.INTER_LINEAR)
            if depth.shape[:2] != (EXPORT_HEIGHT, EXPORT_WIDTH):
                # nearest-neighbor: avoid interpolating real depth values
                # together with invalid (0) ones into meaningless numbers
                depth = cv2.resize(depth, (EXPORT_WIDTH, EXPORT_HEIGHT), interpolation=cv2.INTER_NEAREST)
            frame_rgb[cam] = rgb
