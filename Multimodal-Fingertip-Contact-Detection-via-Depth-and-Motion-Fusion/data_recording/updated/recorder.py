...
from keyboard_layout import load_keyboard_layout, find_key_at_point, draw_keyboard_overlay
...
class Recorder:
    ...
    def _save_frame_set(self, synced):
        ...
        for name, data in synced.items():
            cam_type = self.sync.camera_type(name)
            if cam_type == "depth":
                color, depth_raw_m, depth_filt_m, _ts = data
                self._save_depth_frame(name, frame_id, color, depth_raw_m)
            else:
                frame, _ts = data
                self._save_rgb_frame(name, frame_id, frame)

    def _save_depth_frame(self, name, frame_id, color, depth_raw_m):
        ...
        # Training target: RAW depth, rounded (not truncated) to mm.
        depth_mm = np.round(depth_raw_m * 1000.0).astype(np.uint16)
    ...
    def _show_preview(self, synced, all_hands=None):
        ...
        for name, data in synced.items():
            ...
            # PATCH: draw the full annotated keyboard grid on the primary
            # depth camera's preview at all times (not just under a
            # detected fingertip), so you can visually confirm the
            # physical keyboard sheet is positioned correctly before and
            # during recording.
            if name == self.primary_depth_cam_name and self.keyboard_layout:
                display = draw_keyboard_overlay(display, self.keyboard_layout)
