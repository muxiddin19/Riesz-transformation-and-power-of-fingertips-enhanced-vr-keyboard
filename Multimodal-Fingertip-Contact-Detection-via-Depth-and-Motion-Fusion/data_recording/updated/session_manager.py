...
class SessionManager:
    ...
    def start_new_session(self, camera_configs):
        ...
        # PATCH: auto-load joint extrinsic calibration (RGB cams -> depth
        # cam), written by calibrate_cameras.py --extrinsics, into this
        # session's camera_configs so it's recorded in metadata.json.
        # Safe to skip -- if the file doesn't exist yet, sessions just
        # have no extrinsics_to field, same as before this patch.
        extr_path = os.path.join(config.CALIBRATION_DIR, "extrinsics.json")
        if os.path.exists(extr_path):
            with open(extr_path) as f:
                extrinsics = json.load(f)
            for cam_cfg in camera_configs:
                if cam_cfg["name"] in extrinsics:
                    cam_cfg["extrinsics_to"] = extrinsics[cam_cfg["name"]]
        else:
            print(f"[SESSION] no joint extrinsics found at {extr_path} -- "
                  f"cameras will have no extrinsics_to field. Run "
                  f"calibrate_cameras.py --extrinsics before recording if "
                  f"you need cross-camera reprojection.")
