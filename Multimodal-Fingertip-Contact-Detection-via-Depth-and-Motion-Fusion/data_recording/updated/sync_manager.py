...
class MultiCameraSync:
    ...
    def _build_camera(self, cam_cfg):
        ...
        # PATCH: per-camera resolution override -- lets specific cameras
        # request less USB bandwidth (fewer pixels/frame) than the global
        # RGB_WIDTH/RGB_HEIGHT default, for cameras that stall under
        # multi-device USB contention.
        width = cam_cfg.get("width", self.rgb_width)
        height = cam_cfg.get("height", self.rgb_height)
        return RGBCamera(cam_cfg["name"], cam_cfg["source"], width, height,
                          self.fps, backend, fourcc)
