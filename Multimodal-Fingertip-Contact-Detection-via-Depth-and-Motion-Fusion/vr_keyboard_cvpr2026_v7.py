...
class VelocityBasedContactDetector:
    ...
    def __init__(
        ...
        velocity_threshold_approach: float = 5.0, # was 8.0
        velocity_threshold_stop: float = 4.0,  # was 6.0
        ...
        cooldown_frames: int =15,  # ← Increase from 15
        confidence_threshold: float = 0.50  # ← Increase from 0.55
    ):
    ...
    def compute_natural_tap_quality(self, finger_id: str) -> Tuple[float, Dict[str, Any]]:
        debug = {}
        if finger_id not in self.distance_history or len(self.distance_history[finger_id]) < 5:
            return 0.0, debug
        distances = list(self.distance_history[finger_id])[-12:]
        velocities = list(self.velocity_history.get(finger_id, []))[-12:]
        if len(velocities) < 4 or len(distances) < 5:
            return 0.0, debug

        vy = [v[1] for v in velocities]
        
        # Clamp negative distances to 0
        dist_clamped = [max(0.0, d) for d in distances]
        
        # ============================================================
        # THE KEY INSIGHT: Compute depth_delta during approach phase
        # A real tap shows distance DECREASING while vy is positive
        # A hover shows distance staying flat even with vy noise
        # ============================================================
        
        # Find the approach phase: where vy > 10 (real downward motion)
        approach_frames = [(i, vy[i], dist_clamped[i]) for i in range(len(vy)) 
                          if vy[i] > 6.0 and i < len(dist_clamped)]
        if len(approach_frames) < 1:
            debug['reject'] = 'no_approach'
            return 0.0, debug
        
        # How much did depth change during approach?
        first_approach_dist = approach_frames[0][2]
        last_approach_dist = approach_frames[-1][2]
        depth_delta = first_approach_dist - last_approach_dist  # positive = got closer
        
        # Peak velocity during approach
        peak_vy = max(f[1] for f in approach_frames)
        
        # Current distance (most recent)
        current_dist = dist_clamped[-1]
        min_dist = min(dist_clamped)
        
        debug.update({
            'peak_vy': round(peak_vy, 1),
        ...
        # ============================================================
        # GATE 1: Must have real depth change during approach
        # This is what separates taps from hovers
        # A real tap: depth_delta > 0.5cm (finger moved toward surface)
        # A hover: depth_delta ≈ 0 (finger stayed at same height)
        # ============================================================
        if depth_delta < 0.10:
            debug['reject'] = 'no_depth_change'
            return 0.0, debug
        
        # GATE 2: Must currently be close to surface
        if min_dist > 2.0:
            debug['reject'] = 'never_reached_surface'
            return 0.0, debug
        
        # GATE 3: Must have meaningful velocity
        if peak_vy < 10.0:
            debug['reject'] = 'weak_velocity'
            return 0.0, debug
        ...
        # How much the finger moved toward surface (0.5cm = ok, 2cm = great)
        delta_q = min(1.0, depth_delta / 1.5)
        
        # How close to the surface (0 = touching, 2.5 = barely close enough)
        proximity_q = min(1.0, (3.0 - min_dist) / 2.5)
        
        # Velocity quality
        vel_q = min(1.0, peak_vy / 40.0)
        
        # Post-peak analysis (rebound bonus)
        min_idx = int(np.argmin(dist_clamped))
        went_down = any(v > 6.0 for v in vy[:min_idx + 1])   # approached before bottom
        came_up   = any(v < -3.0 for v in vy[min_idx:])      # lifted after bottom
        is_tap_motion = went_down and came_up
        has_rebound = came_up
        # A real tap descends from height. If the finger never left the
        # surface in this window (max distance stayed low), it's a rest/drag,
        # not a tap — block the "arrived" shortcut.
        max_dist = max(dist_clamped)
        descended_from_height = max_dist > 3.0
        arrived = went_down and (min_dist < 1.5) and descended_from_height
        rebound_bonus = 0.20 if (is_tap_motion or arrived) else 0.0
        
        quality = (0.40 * delta_q + 
                   0.30 * proximity_q + 
                   0.20 * vel_q + 
                   0.10 + rebound_bonus)  # base 0.10 for passing all gates
        
        # Penalty if currently moving upward (already bounced, stale)
        if vy[-1] < -10:
            quality *= 0.6
        ...
        debug.update({
            ...
            'proximity_q': round(proximity_q, 2),
            'vel_q': round(vel_q, 2),
            'rebound': has_rebound,
    ...
    def get_smoothed_depth(self, finger_id: str) -> float:
        ...
        cur = hist[-1]
        if abs(cur - med) > 0.08:      # >4cm jump in one frame = noise, ignore it
            cur = med
        alpha = 0.55
        smoothed = hist[0]
        for v in hist[1:-1]:
            smoothed = alpha * v + (1 - alpha) * smoothed
        smoothed = alpha * cur + (1 - alpha) * smoothed
        return smoothed
    def _init_finger(self, finger_id: str):
        ...
        if finger_id not in self.position_history:
            ...
            self.distance_history[finger_id] = deque(maxlen=max(10, self.history_size))
    ...
    def update_history(
        ...
        if len(self.position_history[finger_id]) > 0:
    ...
    def check_contact(
        ...
        if distance_cm > 3.0:   # relaxed — impact/min_dist logic handles precision
            debug_info['reject'] = 'too_far_now'
        ...
        # Require a real peak velocity this window — a resting/drifting finger
        # has low peak. Deliberate taps spike well above this.
        if debug_info.get('peak_vy', 0) < 14.0:
            debug_info['reject'] = 'no_real_tap'
            return (False, confidence, debug_info)

        # Block stale re-fires: require the finger to have been clearly UP
        # somewhere in this window (real new tap), not just resting near surface.
        if debug_info.get('depth_delta', 0) < 0.25:
            debug_info['reject'] = 'no_fresh_descent'
            return (False, confidence, debug_info)
        
        # Trigger once per tap
        if confidence >= self.confidence_threshold:
            if not self.tap_triggered.get(finger_id, False):
                ...
                # Clear ALL histories so window cannot re-trigger
                # self.velocity_history[finger_id].clear()
                # self.position_history[finger_id].clear()
                
        
        else:
            is_contact = False
        
        # Reset trigger after finger lifts away enough
        if distance_cm > 2.5:
            self.tap_triggered[finger_id] = False
        ...
        if debug and (is_contact or confidence > 0.25):
            print(
                f"[TAPDBG] q={confidence:.2f} dist={distance_cm:.2f}cm "
                f"min={debug_info.get('min_dist', -1):.2f}cm "
                f"peak={debug_info.get('peak_vy', 0):.1f} "
                f"collapse={debug_info.get('collapse', 0):.2f} "
                f"rebound={debug_info.get('rebound', False)} "
...
class VRKeyboardCVPR2026:
    ...
    def __init__(
        ...
        if DEPTH_MODEL_AVAILABLE:
            ...
            checkpoint = r"D:\DL\Research\Multimodal-Fingertip-Contact-Detection-via-Depth-and-Motion-Fusion\checkpoints\depth_anything_v2_vits_d405_finetuned.pth" 
            # checkpoint = r"D:\DL\vscode\Pretrained_weights\depth_anything_v2_vits.pth"
    ...
    custom_checkpoint=checkpoint
        ...
        self.contact_detector = VelocityBasedContactDetector(
            history_size=12,
            contact_entry_threshold_cm=3.0, 
            contact_exit_threshold_cm=4.5,
            cooldown_frames=11,             # 15 frames = 0.5s lockout
            min_peak_velocity=16.0,         # Reject tiny tremors
            velocity_threshold_approach=8.0,
            velocity_threshold_stop=2.0,
            confidence_threshold=0.45       # High threshold for high precision
        )
        ...
        if not self.cap.isOpened():
            raise RuntimeError(f"Cannot open camera {1}")
    ...
    def _find_key_at_point(self, x: int, y: int, margin_px: int = 14) -> Tuple[Optional[Dict], float]:
        """
        ...
        for key in self.keys:
            ...
            # = 0 on edge
            # < 0 outside polygon
            signed_dist = cv2.pointPolygonTest(corners, point, True)
            ...
            if signed_dist >= -local_margin and signed_dist > best_score:
                best_score = signed_dist
                best_key = key

        return best_key, best_score

    def auto_calibrate_keyboard(self, depth_map: np.ndarray, known_distance_cm: float = 31.0):
        print("\n" + "=" * 70)
    ...
    def _check_key_press(
        ...
        if distance_cm < -2.0 or distance_cm > 6.0:
        ...
        if is_contact:
            ...
            best_key, key_score = self._find_key_at_point(contact_x, contact_y, margin_px=22)
            ...
            if best_key is not None:
                ...
                debug_info['rejected_no_key_polygon'] = True
    ...
    def run(self):
        ...
        for _ in range(30):  # Discard first 30 frames
            self.cap.read()  # Wait for camera to stabilize

        # Capture first frame
        ret, frame = self.cap.read()
        if ret:
            depth_map = self._estimate_depth(frame)
            self.auto_calibrate_keyboard(depth_map, known_distance_cm=31.0)
        else:
        ...
        try:
            while True:
                ...
                #             distance_cm = float(user_input) if user_input else 31.0
                #             self.auto_calibrate_keyboard(depth_map, distance_cm)
                #         except ValueError:
                #             print("[ERROR] Invalid input, using default 37cm")
                #             self.auto_calibrate_keyboard(depth_map, 31.0)
                #     else:
